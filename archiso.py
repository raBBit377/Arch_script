...
def loading_animation():
    ...
    while not animation_event.is_set():  # Поки сигналізація не встановлена
        sys.stdout.write("\r" + "Running command " + animation[i])
        sys.stdout.flush()
        i = (i + 1) % len(animation)
        time.sleep(0.5)
...
def run_command(command):
    sys.stdout.write(colors.fg.green + "Running command: " + colors.reset + command + " " + "[#    ]")
    sys.stdout.flush()
    ...
    try:
        ...
        animation_event.clear()  # Скидаємо сигналізацію анімації
        ...
        animation_event.set()  # Встановлюємо сигналізацію анімації

        animation_thread.join()

        sys.stdout.write("\r" + " " * 50 + "\r")  # Очищення рядка виводу
        sys.stdout.flush()
