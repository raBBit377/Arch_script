...
def arch_system():
    ...
    run_command(r"sed -i 's/^# *\(en_US.UTF-8 UTF-8\)/\1/' /etc/locale.gen")
    run_command(r"sed -i 's/^# *\(uk_UA.UTF-8 UTF-8\)/\1/' /etc/locale.gen")
    run_command(r"sed -i 's/^# *\(ru_RU.UTF-8 UTF-8\)/\1/' /etc/locale.gen")
